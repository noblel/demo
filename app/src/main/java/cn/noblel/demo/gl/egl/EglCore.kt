...
class EglCore @JvmOverloads constructor(sharedContext: EGLContext? = null, flags: Int = 0) {
    ...
    private fun getConfig(flags: Int, version: Int): EGLConfig? {
        ...
        val attribList = intArrayOf(EGL14.EGL_RED_SIZE, 8, EGL14.EGL_GREEN_SIZE, 8, EGL14.EGL_BLUE_SIZE, 8,
    ...
    /**
     * Discards all resources held by this class, notably the EGL context.  This must be called from the thread where
    ...
    /**
     * Destroys the specified surface.  Note the EGLSurface won't actually be destroyed if it's still current in a
     * context.
     */
    ...
    fun isCurrent(eglSurface: EGLSurface): Boolean {
        return mEGLContext == EGL14.eglGetCurrentContext() && (eglSurface
    ...
    companion object {
        ...
        /**
         * Constructor flag: surface must be recordable.  This discourages EGL from using a pixel format that cannot be
         * converted efficiently to something usable by the video encoder.
         */
